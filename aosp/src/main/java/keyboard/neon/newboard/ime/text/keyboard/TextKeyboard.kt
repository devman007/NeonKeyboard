...
import keyboard.neon.newboard.ime.keyboard.Keyboard
import keyboard.neon.newboard.ime.keyboard.KeyboardView
import keyboard.neon.newboard.ime.popup.PopupMapping
...
class TextKeyboard(
    ...
    companion object {
        fun layoutDrawableBounds(key: TextKey, factor: Double) {
            layoutForegroundBounds(
                key,
                key.visibleDrawableBounds,
                0.21 * (1.0 / (factor)),
                isLabel = false
            )
        }
        ...
        private fun layoutForegroundBounds(
            key: TextKey,
            bounds: Rect,
            factor: Double,
            isLabel: Boolean
            ...
            bounds.apply {
                ...
                if (w < h) {
                    xOffset = factor * w
                    yOffset =
                        if ((key.computedData.code == KeyCode.SPACE || key.computedData.code == KeyCode.CJK_SPACE) && isLabel) {
                            xOffset
                        } else {
                            (h - (w - 2.0 * xOffset)) / 2.0
                        }
...
//                                (w - (h - 2.0 * yOffset)) / 3.0  /// specific handling for space key when it contains drawable
//                            else
                                (w - (h - 2.0 * yOffset)) / 2.0
                        }
                }
    ...
    override fun layout(keyboardView: KeyboardView) {
        ...
        val rowMarginV =
            (keyboardHeight - desiredTouchBounds.height() * rowCount.toDouble()) / (rowCount - 1).coerceAtLeast(
                1
            ).toDouble()
        ...
        for ((r, row) in rows().withIndex()) {
            ...
            if (requestedWidth <= availableWidth) {
                ...
                for ((k, key) in row.withIndex()) {
                    ...
                    layoutDrawableBounds(
                        key,
                        keyboardView.fontSizeMultiplier * getDrawableSize((row[k].computedData.code))
                    )
                ...
                for ((k, key) in row.withIndex()) {
                    ...
                    /// calculating the drawable size when directly drawing from the path
