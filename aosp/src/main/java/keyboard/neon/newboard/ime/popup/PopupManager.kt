...
import keyboard.neon.newboard.ime.media.emoji.EmojiKey
import keyboard.neon.newboard.ime.media.emoji.EmojiKeyData
import keyboard.neon.newboard.ime.media.emoji.EmojiKeyboardView
...
class PopupManager<V : View>(
    ...
    private fun isSuitableForBasicPopup(key: Key): Boolean {
        return if (key is TextKey) {
            ...
            (c > KeyCode.SPACE || c == KeyCode.UNSPECIFIED) && c != KeyCode.MULTIPLE_CODE_POINTS && c != KeyCode.CJK_SPACE
        } else {
            true
        }
    }
    ...
    private fun isSuitableForExtendedPopup(key: Key): Boolean {
        return if (key is TextKey) {
            val c = key.computedData.code
            (c > KeyCode.SPACE || c == KeyCode.UNSPECIFIED) && c != KeyCode.MULTIPLE_CODE_POINTS && c != KeyCode.CJK_SPACE || exceptionsForKeyCodes.contains(c)
        } else {
            true
        }
    }
