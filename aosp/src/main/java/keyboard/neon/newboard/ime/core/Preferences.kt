...
import keyboard.neon.newboard.ime.text.gestures.DistanceThreshold
import keyboard.neon.newboard.ime.text.gestures.SwipeAction
import keyboard.neon.newboard.ime.text.gestures.VelocityThreshold
import keyboard.neon.newboard.ime.text.key.KeyHintConfiguration
import keyboard.neon.newboard.ime.text.key.KeyHintMode
import keyboard.neon.newboard.ime.text.key.UtilityKeyAction
import keyboard.neon.newboard.ime.text.smartbar.CandidateView
import keyboard.neon.newboard.ime.theme.ThemeMode
import keyboard.neon.newboard.util.TimeUtil
import keyboard.neon.newboard.util.VersionName
...
class Preferences(
    ...
    companion object {
        ...
        private val OLD_SUBTYPES_REGEX =
            """^([\-0-9]+/[\-a-zA-Z0-9]+(/[a-zA-Z_]+)?/[a-zA-Z_]+[;]*)+${'$'}""".toRegex()
     ...
     */
    class Fonts(private val prefs: Preferences) {
        companion object {
            const val FONT_NAME = "font__name"
        }

        var fontName: String
            get() = prefs.getPref(FONT_NAME, "qwerty")
            set(v) = prefs.setPref(FONT_NAME, v)
    }
    ...
    class Correction(private val prefs: Preferences) {
        companion object {
            ...
            const val AUTO_SPACE_AFTER_DOT = "correction__auto_space"
        }

        var autoSpace: Boolean
            get() = prefs.getPref(AUTO_SPACE_AFTER_DOT, true)
            set(v) = prefs.setPref(AUTO_SPACE_AFTER_DOT, v)
    ...
    class Devtools(private val prefs: Preferences) {
        companion object {
            ...
            const val OVERRIDE_WS_MIN_HEAP_RES =
                "devtools__override_word_suggestions_min_heap_restriction"
        }
...
//    }
    ...
    class Gestures(private val prefs: Preferences) {
        ...
        var spaceBarLongPress: SwipeAction
            get() = SwipeAction.fromString(
                prefs.getPref(
                    SPACE_BAR_LONG_PRESS,
        ...
        var spaceBarSwipeLeft: SwipeAction
            get() = SwipeAction.fromString(prefs.getPref(SPACE_BAR_SWIPE_LEFT, "move_cursor_left"))
    ...
    class InputFeedback(private val prefs: Preferences) {
        companion object {
            ...
            const val AUDIO_FEAT_KEY_REPEATED_ACTION =
                "input_feedback__audio_feat_key_repeated_action"
            const val AUDIO_FEAT_GESTURE_SWIPE = "input_feedback__audio_feat_gesture_swipe"
            const val AUDIO_FEAT_GESTURE_MOVING_SWIPE =
                "input_feedback__audio_feat_gesture_moving_swipe"

            const val HAPTIC_ENABLED = "input_feedback__haptic_enabled"
            const val HAPTIC_IGNORE_SYSTEM_SETTINGS =
                "input_feedback__haptic_ignore_system_settings"
            ...
            const val HAPTIC_FEAT_KEY_REPEATED_ACTION =
                "input_feedback__haptic_feat_key_repeated_action"
            const val HAPTIC_FEAT_GESTURE_SWIPE = "input_feedback__haptic_feat_gesture_swipe"
            const val HAPTIC_FEAT_GESTURE_MOVING_SWIPE =
                "input_feedback__haptic_feat_gesture_moving_swipe"
        }
        ...
        var audioIgnoreSystemSettings: Boolean
            get() = prefs.getPref(AUDIO_IGNORE_SYSTEM_SETTINGS, true)
            set(v) = prefs.setPref(AUDIO_IGNORE_SYSTEM_SETTINGS, v)
        var audioVolume: Int
            get() = prefs.getPref(AUDIO_VOLUME, 90)
        ...
        var hapticIgnoreSystemSettings: Boolean
            get() = prefs.getPref(HAPTIC_IGNORE_SYSTEM_SETTINGS, true)
    ...
    class Keyboard(private val prefs: Preferences) {
        companion object {
            ...
            const val SPACE_BAR_SWITCHES_TO_CHARACTERS =
                "keyboard__space_bar_switches_to_characters"
        ...
        var hintedNumberRowMode: KeyHintMode
            get() = KeyHintMode.fromString(
                prefs.getPref(
                    HINTED_NUMBER_ROW_MODE,
                    KeyHintMode.ENABLED_ACCENT_PRIORITY.toString()
                )
            )
        ...
        var landscapeInputUiMode: LandscapeInputUiMode
            get() = LandscapeInputUiMode.fromString(
                prefs.getPref(
                    LANDSCAPE_INPUT_UI_MODE,
                    LandscapeInputUiMode.DYNAMICALLY_SHOW.toString()
                )
            )
        ...
        var utilityKeyAction: UtilityKeyAction
            get() = UtilityKeyAction.fromString(
                prefs.getPref(
                    UTILITY_KEY_ACTION,
                    UtilityKeyAction.DYNAMIC_SWITCH_LANGUAGE_EMOJIS.toString()
                )
            )
...
//    }
    ...
    class Suggestion(private val prefs: Preferences) {
        companion object {
            const val API30_INLINE_SUGGESTIONS_ENABLED =
                "suggestion__api30_inline_suggestions_enabled"
        ...
        var displayMode: CandidateView.DisplayMode
            get() = CandidateView.DisplayMode.fromString(
                prefs.getPref(
                    DISPLAY_MODE,
                    CandidateView.DisplayMode.DYNAMIC_SCROLLABLE.toString()
                )
            )
    ...
    class Theme(private val prefs: Preferences) {
        ...
        var mode: ThemeMode
            ...
            get() = ThemeMode.fromString(prefs.getPref(MODE, ThemeMode.ALWAYS_DAY.toString()))
            set(v) = prefs.setPref(MODE, v)
        var dayThemeRef: String
            get() = prefs.getPref(DAY_THEME_REF, "assets:ime/theme/theme1.json")
        ...
        var nightThemeRef: String
            get() = prefs.getPref(NIGHT_THEME_REF, "assets:ime/theme/theme1.json")
    ...
    class Clipboard(private val prefs: Preferences) {
        ...
        var syncToSystem: Boolean
            get() = prefs.getPref(SYNC_TO_SYSTEM, true)
        ...
        var enableHistory: Boolean
            get() = prefs.getPref(ENABLE_HISTORY, true)
            set(v) = prefs.setPref(ENABLE_HISTORY, v)

        var cleanUpOld: Boolean
            get() = prefs.getPref(CLEAN_UP_OLD, true)
