...
class EmojiKeyView(
    ...
    key: EmojiKey,
    val emojiClick: ((EmojiKeyData) -> Unit)? = null
    ...
    init {
        ...
        triangleDrawable =
            ContextCompat.getDrawable(context, R.drawable.triangle_bottom_right)?.mutate()
...
//                    context.showToast("click")
                    ...
                    emojiKeyboardView.apply {
                        isScrollBlocked = true
                        popupManager.show(key, KeyHintConfiguration.HINTS_DISABLED)
                        popupManager.extend(key, KeyHintConfiguration.HINTS_DISABLED)
                    }
            ...
            MotionEvent.ACTION_CANCEL -> {
                ...
                if (event.actionMasked != MotionEvent.ACTION_CANCEL &&
                    ...
                    (retData as? EmojiKeyData)?.let {
                        emojiClick?.invoke(it)
                        florisboard?.mediaInputManager?.sendEmojiKeyPress(it)
                    }
                    performClick()
                }
                if (event.actionMasked == MotionEvent.ACTION_CANCEL) {
    ...
    override fun onThemeUpdated(theme: Theme) {
        triangleDrawable?.setTint(
            theme.getAttr(Theme.Attr.MEDIA_FOREGROUND_ALT).toSolidColor().color
        )
    }
