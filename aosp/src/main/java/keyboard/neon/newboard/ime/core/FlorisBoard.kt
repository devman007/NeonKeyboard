...
import keyboard.neon.newboard.banads.AdaptiveAds
...
import keyboard.neon.newboard.ime.clip.ClipboardInputManager
import keyboard.neon.newboard.ime.clip.FlorisClipboardManager
import keyboard.neon.newboard.ime.keyboard.InputFeedbackManager
import keyboard.neon.newboard.ime.keyboard.KeyboardState
import keyboard.neon.newboard.ime.keyboard.updateKeyboardState
import keyboard.neon.newboard.ime.landscapeinput.LandscapeInputUiMode
import keyboard.neon.newboard.ime.media.MediaInputManager
import keyboard.neon.newboard.ime.onehanded.OneHandedMode
import keyboard.neon.newboard.ime.popup.PopupLayerView
import keyboard.neon.newboard.ime.text.TextInputManager
import keyboard.neon.newboard.ime.text.composing.Appender
import keyboard.neon.newboard.ime.text.composing.Composer
import keyboard.neon.newboard.ime.text.gestures.SwipeAction
import keyboard.neon.newboard.ime.text.key.CurrencySet
...
import keyboard.neon.newboard.util.*
 ...
 */
open class FlorisBoard : InputMethodService(), LifecycleOwner,
    FlorisClipboardManager.OnPrimaryClipChangedListener,
    ThemeManager.OnThemeUpdatedListener {

    private val serviceLifecycleDispatcher: ServiceLifecycleDispatcher =
...
//        CrashUtility.handleStagedButUnhandledExceptions()
    ...
    override fun onCreateExtractTextView(): View? {
        ...
        eel.viewTreeObserver.addOnGlobalLayoutListener(object :
    ...
    override fun onUpdateExtractingVisibility(ei: EditorInfo?) {
        isExtractViewShown =
            activeState.isRichInputEditor && when (prefs.keyboard.landscapeInputUiMode) {
...
//            }
    ...
    override fun onThemeUpdated(theme: Theme) {
        ...
        val tabSelectedTextColor =
            theme.getAttr(Theme.Attr.TAB_SELECTED_TEXT_COLOR).toSolidColor().color
        val tabUnSelectedTextColor =
            theme.getAttr(Theme.Attr.TAB_UNSELECTED_TEXT_COLOR).toSolidColor().color

//                    val tabUnSelected =
//                        theme.getAttr(Theme.Attr.WINDOW_COLOR_PRIMARY_DARK).toSolidColor().color
        ...
        w.navigationBarColor =
            theme.getAttr(Theme.Attr.WINDOW_NAVIGATION_BAR_COLOR).toSolidColor().color
...
//        uiBinding?.inputView?.setBackgroundColor(theme.getAttr(Theme.Attr.KEYBOARD_BACKGROUND).toSolidColor().color)
        if (theme.isNeon) {
            uiBinding?.text?.apply {


                uiBinding?.inputView?.background = ColorDrawable(Color.BLACK)
                uiBinding?.clipboard?.clipInput?.background
//                uiBinding?.text?.background = ColorDrawable(Color.BLACK)

                uiBinding?.media?.root?.background =
                    (theme.getAttr(Theme.Attr.KEYBOARD_BG_DRAWABLE).toDrawable()
                        .getDrawableId(this@FlorisBoard))

                uiBinding?.media?.mediaInputTabs?.tabTextColors =
                    createColorStateList(tabSelectedTextColor, tabUnSelectedTextColor)

                optionsTopBar.background = ColorDrawable(Color.BLACK)
                mainKeyboardView.background =
                    (theme.getAttr(Theme.Attr.KEYBOARD_BG_DRAWABLE).toDrawable()
                        .getDrawableId(this@FlorisBoard))

                keyboardBgAnim.apply {
                    setAnimation(theme.animationName)
                    isVisible = true
                    playAnimation()
                }
            ...
            uiBinding?.apply {
                ...
                text.apply {
                    ...
                    optionsTopBar.background = ColorDrawable(Color.TRANSPARENT)
                    mainKeyboardView.background = ColorDrawable(Color.TRANSPARENT)
                    root.background = ColorDrawable(Color.TRANSPARENT)

                    inputView.background =
                        (theme.getAttr(Theme.Attr.KEYBOARD_BG_DRAWABLE).toDrawable()
                            .getDrawableId(this@FlorisBoard))
//                    inputViewBgLayout.background =
//                        (theme.getAttr(Theme.Attr.KEYBOARD_BG_DRAWABLE).toDrawable()
//                            .getDrawableId(this@FlorisBoard))
                }
                media.root.background = ColorDrawable(Color.TRANSPARENT)
...
//                        .getDrawableId(this@FlorisBoard))
        ...
        extractEditLayout.get()?.let { eel ->
            ...
            eel.setBackgroundColor(
                theme.getAttr(Theme.Attr.EXTRACT_EDIT_LAYOUT_BACKGROUND).toSolidColor().color
            )
            eel.findViewById<ExtractEditText>(android.R.id.inputExtractEditText)?.let { eet ->
                ...
                eet.background =
                    ContextCompat.getDrawable(this, R.drawable.edit_text_background)?.also { d ->
                ...
                eet.setTextColor(
                    theme.getAttr(Theme.Attr.EXTRACT_EDIT_LAYOUT_FOREGROUND).toSolidColor().color
                )
                eet.setHintTextColor(
                    theme.getAttr(Theme.Attr.EXTRACT_EDIT_LAYOUT_FOREGROUND_ALT)
                        .toSolidColor().color
                )
                eet.highlightColor =
                    theme.getAttr(Theme.Attr.WINDOW_COLOR_PRIMARY).toSolidColor().color
            ...
            eel.findViewWithType(Button::class)?.let { btn ->
                btn.background =
                    ContextCompat.getDrawable(this, R.drawable.shape_rect_rounded)?.also { d ->
                        DrawableCompat.setTint(
                            d,
                            theme.getAttr(Theme.Attr.EXTRACT_ACTION_BUTTON_BACKGROUND)
                                .toSolidColor().color
                        )
                    }
                btn.setTextColor(
                    theme.getAttr(Theme.Attr.EXTRACT_ACTION_BUTTON_FOREGROUND).toSolidColor().color
                )
            }
            eel.invalidate()
        }
...
//        applicationContext.startActivity(i)
    ...
    private fun switchToPrevSubtype() {
        flogInfo(LogTopic.IMS_EVENTS)
...
//    fun changeFont(subtypeName: String) {
//        flogInfo(LogTopic.IMS_EVENTS)
//        activeSubtype = Subtype.font(subtypeName)
//        onSubtypeChanged(activeSubtype, true)
//    }
    ...
    data class ImeConfig(
        ...
        init {
            ...
            for (defaultSubtype in defaultSubtypes) {
                tmpSubtypeList.add(
                    Pair(
                        defaultSubtype.locale.localeTag(),
                        defaultSubtype.locale.displayName()
                    )
                )
            }
    ...
    fun showBanner(bannerLayout: FrameLayout) {
