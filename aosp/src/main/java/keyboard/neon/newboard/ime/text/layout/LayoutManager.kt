...
import keyboard.neon.newboard.debug.flogWarning
import keyboard.neon.newboard.ime.core.Preferences
import keyboard.neon.newboard.ime.core.Subtype
import keyboard.neon.newboard.ime.keyboard.DefaultComputingEvaluator
import keyboard.neon.newboard.ime.popup.PopupExtension
...
import keyboard.neon.newboard.ime.text.keyboard.KeyboardMode
import keyboard.neon.newboard.ime.text.keyboard.TextKey
import keyboard.neon.newboard.ime.text.keyboard.TextKeyData
import keyboard.neon.newboard.ime.text.keyboard.TextKeyboard
 ...
 */
class LayoutManager {
    val assetManager get() = AssetManager.default()
    ...
    private val indexedLayoutRefs: EnumMap<LayoutType, MutableList<Pair<FlorisRef, LayoutMetaOnly>>> =
        EnumMap(LayoutType::class.java)
...
//        val ref = FlorisRef.assets("ime/text/${ltn.type}/${ltn.name}.json")
        val ref = FlorisRef.assets("ime/text/${ltn.type}/${ltn.name}.json")
//        val ref = FlorisRef.assets("ime/text/arabic.json")
    ...
    private fun loadExtendedPopupsAsync(subtype: Subtype? = null): Deferred<Result<PopupExtension>> =
        ioScope.async {
            val ref: FlorisRef = if (subtype == null) {
                FlorisRef.assets("${PopupManager.POPUP_EXTENSION_PATH_REL}/\$default.json")
            } else {
                val tempRef =
                    FlorisRef.assets("${PopupManager.POPUP_EXTENSION_PATH_REL}/${subtype.locale.languageTag()}.json")
    ...
    private suspend fun mergeLayoutsAsync(
        ...
        val modifierLayout: Layout? = loadLayoutAsync(modifierToLoad).await().getOrNull()
...
//                        for (layoutName in listOf("qwerty")) {
//                            val index: Int =
//                                indexedList.indexOfFirst { it.second.name == layoutName }
//                            if (index > 0) {
