...
import keyboard.neon.newboard.common.Pointer
import keyboard.neon.newboard.common.PointerMap
import keyboard.neon.newboard.common.ViewUtils
import keyboard.neon.newboard.debug.LogTopic
import keyboard.neon.newboard.debug.flogDebug
import keyboard.neon.newboard.debug.flogError
import keyboard.neon.newboard.debug.flogInfo
import keyboard.neon.newboard.ime.core.InputKeyEvent
...
import keyboard.neon.newboard.ime.text.gestures.GlideTypingGesture
import keyboard.neon.newboard.ime.text.gestures.GlideTypingManager
import keyboard.neon.newboard.ime.text.gestures.SwipeAction
import keyboard.neon.newboard.ime.text.gestures.SwipeGesture
import keyboard.neon.newboard.ime.text.key.KeyCode
import keyboard.neon.newboard.ime.text.key.KeyHintConfiguration
import keyboard.neon.newboard.ime.text.key.KeyType
import keyboard.neon.newboard.ime.text.key.KeyVariation
import keyboard.neon.newboard.ime.theme.Theme
import keyboard.neon.newboard.ime.theme.ThemeValue
import keyboard.neon.newboard.util.VectorDrawableParser
import keyboard.neon.newboard.util.getThemeColor
...
class TextKeyboardView : KeyboardView, SwipeGesture.Listener, GlideTypingGesture.Listener,
    ...
    private val childRect = Rect(100, 10, 300, 400)
    ...
    private var cachedState: KeyboardState =
        KeyboardState.new(maskOfInterest = KeyboardState.INTEREST_TEXT)
    ...
    private val glideDataForDrawing: MutableList<Pair<GlideTypingGesture.Detector.Position, Long>> =
        mutableListOf()
    private val fadingGlide: MutableList<Pair<GlideTypingGesture.Detector.Position, Long>> =
        mutableListOf()
...
//        setupEraser()
        ...
        //  for transparent the key to show background
        ...
        context.obtainStyledAttributes(attrs, R.styleable.TextKeyboardView).apply {
            ...
            isSmartbarKeyboardView =
                getBoolean(R.styleable.TextKeyboardView_isSmartbarKeyboard, false)
            isLoadingPlaceholderKeyboard =
                getBoolean(R.styleable.TextKeyboardView_isLoadingPlaceholderKeyboard, false)
            recycle()
        }
    ...
    private fun onTouchCancelInternal(event: MotionEvent, pointer: TouchPointer) {
        ...
        if (activeKey != null) {
            activeKey.setPressed(false) { invalidate(activeKey) }
            florisboard.textInputManager.inputEventDispatcher.send(InputKeyEvent.cancel(activeKey.computedData))
            if (popupManager.isSuitableForPopups(activeKey)) {
                popupManager.hide()
            }
            pointer.activeKey = null
            ...
            KeyCode.SPACE, KeyCode.CJK_SPACE -> handleSpaceSwipe(event)
                ...
                initialKey.computedData.code == KeyCode.SHIFT && activeKey?.computedData?.code != KeyCode.SHIFT &&
                    ...
                    activeKey?.let {
                        florisboard.textInputManager.inputEventDispatcher.send(
                            InputKeyEvent.up(
                                popupManager.getActiveKeyData(it, keyHintConfiguration)
                                    ?: it.computedData
                            )
                        )
                    }
                    ...
                    florisboard.activeEditorInstance.apply {
                        ...
                        if (selection.isValid) {
                            selection.updateAndNotify(
                                (selection.end + event.absUnitCountX + 1).coerceIn(
                                    0,
                                    selection.end
                                ),
                                selection.end
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val desiredHeight = if (isSmartbarKeyboardView || isPreviewMode) {
            ...
            (florisboard?.uiBinding?.inputView?.desiredTextKeyboardViewHeight
...
//        }
    ...
    /**
     * measuring the top left right bottom of the key
     */
...
//        )
        ...
        /// laying the child items(TextkeyboardView) in viewgroup on specific position
        rv.layout(
            key.visibleBounds.left,
            key.visibleBounds.top,
            key.visibleBounds.right,
    ...
    private fun prepareKey(key: TextKey, theme: Theme, renderView: TextKeyView) {
        var keyCornerRadius: ThemeValue? = null
        var keyStrokeColor: ThemeValue? = null
        var keyElevation: ThemeValue? = null
        ...
        when {
            isLoadingPlaceholderKeyboard -> {
                shouldShowBorder =
                    theme.getAttr(Theme.Attr.KEY_SHOW_BORDER, themeLabel).toOnOff().state
            ...
            isSmartbarKeyboardView -> {
                ...
                keyCornerRadius =
                    theme.getAttr(Theme.Attr.KEY_CORNER_RADIUS, themeLabel)
                keyStrokeColor =
                    theme.getAttr(Theme.Attr.KEY_STROKE_COLOR, themeLabel)
                keyElevation =
                    theme.getAttr(Theme.Attr.KEY_ELEVATION, themeLabel)
                shouldShowBorder =
                    theme.getAttr(Theme.Attr.KEY_SHOW_BORDER, themeLabel, capsSpecific)
                        .toOnOff().state
                if (key.isPressed && key.isEnabled) {
                    keyBackground =
                        theme.getAttr(Theme.Attr.KEY_BACKGROUND_PRESSED, themeLabel, capsSpecific)
                    keyForeground =
                        theme.getAttr(Theme.Attr.KEY_FOREGROUND_PRESSED, themeLabel, capsSpecific)
                } else {
                    keyBackground =
                        theme.getAttr(Theme.Attr.KEY_BACKGROUND, themeLabel, capsSpecific)
                    keyForeground =
                        theme.getAttr(Theme.Attr.KEY_FOREGROUND, themeLabel, capsSpecific)
                }
            }
        }
        renderView.let { rv ->
            rv.elevation = if (shouldShowBorder) keyElevation.toString().toFloat() else 0.0f

            rv.shouldShowBorder = shouldShowBorder
            rv.bgDrawable.apply {

//                paint.color = theme.getThemeColor(keyBackground, null)!!
                paint.color = keyBackground.toSolidColor().color

                keyCornerRadius?.let {
                    rv.strokeRadius = it.toString()
                        .toFloat()  /// setting the value to the current key for setting setting radius when drawing stroke on canvas
                    setCornerRadius(ViewUtils.dp2px(it.toString().toFloat()))
                }
            }

//            rv.borderPaint.color = keyBackground.toSolidColor().color

//            rv.borderPaint.color = theme.getThemeColor(
//                keyStrokeColor,
//                if (shouldShowBorder) rv.borderPaint else null  /// setting paint null to prevent applying xfermode in case of neon
//            )!!


//                 handle border for simple and neon theme
            rv.borderPaint.color = theme.getThemeColor(
                keyStrokeColor,
                rv.borderPaint,
                shouldShowBorder
...
//            rv.bgDrawable.paint.color = ThemeValue.SolidColor.TRANSPARENT.toSolidColor().color
            rv.labelPaint.let {

                it.color = theme.getThemeColor(keyForeground, it)!!
...
//                }
    ...
    override fun onThemeUpdated(theme: Theme) {
        if (isPreviewMode) {
            cachedTheme = theme
            background =
                (theme.getAttr(Theme.Attr.KEYBOARD_BG_DRAWABLE).toDrawable()
...
//            }
        ...
        if (theme.getAttr(Theme.Attr.GLIDE_TRAIL_COLOR).toSolidColor().color == 0) {
            glideTrailPaint.color =
                theme.getAttr(Theme.Attr.WINDOW_COLOR_PRIMARY).toSolidColor().color
        ...
        for (n in 0 until childCount) {
            ...
            if (rv?.key != null) {
                ...
                prepareKey(
                    rv.key!!,
                    cachedTheme ?: themeManager?.activeTheme ?: Theme.BASE_THEME,
                    rv
                )
                rv.invalidate()
            }
    ...
    fun onDrawComputedKey(canvas: Canvas, key: TextKey, renderView: TextKeyView) {
        ...
        if (label != null) {
            renderView.labelPaint.let {
                ...
                val centerY =
                    key.visibleLabelBounds.exactCenterY() + (it.textSize - it.descent()) / 2
                if (label.contains("\n")) {
        ...
        // CHANGE RADIUS OF KEY FOR SET OVER LAPPING ISSUE
...
//                val centerX = key.visibleBounds.left + key.visibleBounds.width() * 5.0f / 6.0f
                val centerX = key.visibleBounds.left + key.visibleBounds.width() * 5.0f / 6.5f
                val centerY =
...
//            }
            val vectorPath =
                VectorDrawableParser.parsedVectorDrawable(resources, foregroundDrawableId)?.pathData
            val parsedPath = PathParser.createPathFromPathData(vectorPath).apply {

                offset(
                    key.visibleDrawableBounds.exactCenterX(),
                    key.visibleDrawableBounds.exactCenterY()
                )

                fillType = Path.FillType.EVEN_ODD
                /** further handling of size calculations {@link #TextKeyboard.layoutDrawableBounds(key: TextKey, factor: Double)}
                 */
                val src = RectF()
                computeBounds(src, true)
                val resizeMatrix = Matrix().apply {

                    setRectToRect(
                        src,
                        key.visibleDrawableBounds.toRectF(),
                        Matrix.ScaleToFit.CENTER
                    )
                }

                transform(resizeMatrix)

//                val src = RectF()
//                computeBounds(src, true)
//                val bounds = RectF(
//                    src.left - (src.width() * 1.2f),
//                    src.top - (src.height()),
//                    src.right,
//                    src.bottom
//                )
//                val scaleMatrix = Matrix()
//                scaleMatrix.setRectToRect(src, bounds, Matrix.ScaleToFit.CENTER)
...
//                transform(scaleMatrix)
            ...
            if (key.computedData.code == KeyCode.SHIFT) {
                ...
                renderView.labelPaint.style = if ((cachedState.capsLock || !cachedState.caps))
                    Paint.Style.FILL
                else
                    Paint.Style.STROKE
            } else
                renderView.labelPaint.style = Paint.Style.FILL


            canvas.drawPath(parsedPath, renderView.labelPaint)

        }

        /// drawing the stroke around the key for changing color
        // initially it is transparent

//        if (renderView.shouldShowBorder) {
        val round = ViewUtils.dp2px(renderView.strokeRadius)
        canvas.drawRoundRect(key.visibleBounds.toRectF(), round, round, renderView.borderPaint)
//        }
    }
    ...
    private fun computeLabelsAndDrawables(
        ...
        if (data.type == KeyType.CHARACTER && data.code != KeyCode.SPACE && data.code != KeyCode.CJK_SPACE
            ...
            key.computedPopups.getPopupKeys(keyHintConfiguration).hint?.asString(isForDisplay = true)
                .let {
                    key.hintedLabel = it
                }
        } else {
                ...
                KeyCode.SPACE, KeyCode.CJK_SPACE -> {
                    when (computedKeyboard?.mode) {
                        ...
                        KeyboardMode.CHARACTERS -> {
                            key.label = if (isPreviewMode) resources.getString(R.string.space)
                            else
                                florisboard?.activeSubtype?.locale?.displayName()
                        }
                        else -> {
                        }
                    }
    ...
    // WILL CHANGE IT FOR FUTURE IMPLEMENTATION
    override fun onDraw(canvas: Canvas?) {
        super.onDraw(canvas)

//        canvas?.drawRect(childRect, eraser)

//        computedKeyboard?.keys()?.withIndex()?.forEach { (n, _) ->
//
//
//            getChildAt(n)?.let { rv ->
////                key.label?.let { Log.d("label", it) }
////                context.showToast(key.computedData.label)
//
//
//                if (rv is TextKeyView) {
//
...
////                        val vectorPath =
////                            VectorDrawableParser.parsedVectorDrawable(resources, foregroundDrawableId)?.pathData
////                        val parsedPath = PathParser.createPathFromPathData(vectorPath)
////
////                        canvas?.drawPath(parsedPath, rv.labelPaint)
////                    }
//
////                    childRect.set(
////                        key.visibleLabelBounds.left, key.visibleLabelBounds.top,
////                        key.visibleLabelBounds.right, key.visibleLabelBounds.bottom
////                    )
////
////                    val centerX = key.visibleLabelBounds.exactCenterX()
////                    val centerY =
////                        key.visibleLabelBounds.exactCenterY() + (rv.labelPaint.textSize - rv.labelPaint.descent()) / 2
////                    key.label?.let {
//////                        canvas?.drawText(it, centerX, centerY, eraser)
////                        canvas?.drawRect(key.visibleBounds, eraser)
////                    }
//                }
//            }
//        }
