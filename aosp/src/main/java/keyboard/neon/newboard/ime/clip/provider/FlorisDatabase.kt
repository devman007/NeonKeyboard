        ...
        fun fromClipData(data: ClipData, cloneUri: Boolean): ClipboardItem {
            ...
            val uri = if (type == ItemType.IMAGE) {
                if (data.getItemAt(0).uri.authority == FlorisContentProvider.CONTENT_URI.authority || !cloneUri) {
                    ...
                    FlorisBoard.getInstance().contentResolver.insert(
                        FlorisContentProvider.CLIPS_URI,
                        values
                    )
                }
            } else {
            ...
            val text = FlorisBoard.getInstanceOrNull()
                ?.let { data.getItemAt(0).coerceToText(it).toString() } ?: "#ERROR"
...
@Database(entities = [FileUri::class], version = 1, exportSchema = false)
@TypeConverters(Converters::class)
